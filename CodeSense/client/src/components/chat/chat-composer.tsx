    ...
    <div className="shrink-0 border-t bg-background/90 px-4 py-3 backdrop-blur sm:px-6">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="flex items-end gap-2 rounded-xl border bg-card p-2 shadow-sm focus-within:border-primary/40 focus-within:ring-3 focus-within:ring-ring/15">
          <Textarea value={value} onChange={(event) => setValue(event.target.value)} placeholder="Ask CodeSense about this repository..." disabled={disabled} className="max-h-36 min-h-10 flex-1 resize-none border-0 bg-transparent px-2 py-2 shadow-none focus-visible:ring-0" onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submit(); } }} />
