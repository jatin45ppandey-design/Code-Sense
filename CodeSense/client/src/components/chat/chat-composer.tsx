...
export function ChatComposer({ disabled, streaming, error, restoreRequest, onSend, onStop }: { disabled?: boolean; streaming: boolean; error?: string | null; restoreRequest?: { id: number; content: string } | null; onSend: (content: string) => Promise<void>; onStop: () => void }) {
  const [value, setValue] = useState("");
  const textareaRef = useRef<HTMLTextAreaElement>(null);

  useEffect(() => {
    if (!restoreRequest) return;
    setValue(restoreRequest.content);
    ...
    <div className="shrink-0 border-t border-border/80 bg-background/95 px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] shadow-[0_-8px_18px_-16px_rgba(15,23,42,0.45)] backdrop-blur-md sm:px-6">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="flex items-end gap-2 rounded-xl border bg-card p-2 shadow-sm focus-within:border-primary/40 focus-within:ring-3 focus-within:ring-ring/15">
          <Textarea ref={textareaRef} value={value} onChange={(event) => setValue(event.target.value)} placeholder="Ask CodeSense about this repository..." disabled={disabled} className="max-h-40 min-h-12 flex-1 resize-none overflow-y-auto border-0 bg-transparent px-2 py-2.5 leading-6 shadow-none focus-visible:ring-0" onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void submit(); } }} />
