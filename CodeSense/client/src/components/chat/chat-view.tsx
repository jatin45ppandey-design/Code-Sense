...
export function ChatView({ repositoryId }: { repositoryId: string }) {
  ...
  return <div className="flex min-h-[calc(100svh-4.25rem)] flex-1 flex-col"><div className="flex min-h-14 shrink-0 items-center gap-3 border-b px-4"><Button variant="ghost" size="icon-sm" nativeButton={false} render={<Link href={`/repositories/${repo.id}`} />} aria-label="Back to repository"><ArrowLeft /></Button>{ready && <Button variant="ghost" size="icon-sm" className="lg:hidden" onClick={() => setMobileSessionsOpen(true)} aria-label="Open recent questions"><PanelLeft /></Button>}<span className="flex size-8 items-center justify-center rounded-lg border bg-card"><Braces className="size-4 text-primary" /></span><div className="min-w-0 flex-1"><h1 className="truncate text-sm font-semibold">{repo.fullName}</h1><p className="truncate font-mono text-[10px] text-muted-foreground">{repo.defaultBranch} · Ask CodeSense</p></div><StatusBadge status={currentStatus || repo.indexStatus} /></div>{!ready ? <div className="flex flex-1 items-center justify-center p-6"><div className="w-full max-w-md rounded-xl border bg-card p-6 text-center shadow-sm">{currentStatus === "FAILED" ? <AlertTriangle className="mx-auto size-7 text-destructive" /> : currentStatus === "INDEXING" ? <LoaderCircle className="mx-auto size-7 animate-spin text-primary" /> : <Braces className="mx-auto size-7 text-primary" />}<h2 className="mt-4 font-semibold">{currentStatus === "FAILED" ? "Indexing failed" : currentStatus === "INDEXING" ? "Preparing repository context" : "Index this repository to start"}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{currentStatus === "FAILED" ? errorMessage || "The backend did not provide an error message." : currentStatus === "INDEXING" ? `${filesProcessed}${filesTotal ? ` of ${filesTotal}` : ""} files processed. Ask CodeSense unlocks when indexing finishes.` : "CodeSense needs to index repository files before it can answer grounded questions."}</p>{currentStatus === "INDEXING" && <Progress className="mt-5" value={filesTotal ? progress : null} />}{currentStatus !== "INDEXING" && <Button className="mt-5" disabled={index.isPending} onClick={() => index.mutate(repo.id)}>{currentStatus === "FAILED" ? <RotateCw /> : <Play />}{index.isPending ? "Starting..." : currentStatus === "FAILED" ? "Retry indexing" : "Start indexing"}</Button>}{index.error && <p className="mt-3 text-xs text-destructive">{index.error.message}</p>}</div></div> : <div className="flex min-h-0 flex-1 flex-col lg:flex-row"><SessionList repositoryId={repo.id} selectedId={activeId} onSelect={setSelectedId} /><Sheet open={mobileSessionsOpen} onOpenChange={setMobileSessionsOpen}><SheetContent side="left" className="flex w-[min(22rem,90vw)] flex-col p-0 lg:hidden"><SheetHeader className="border-b px-4 py-4 text-left"><SheetTitle>Recent questions</SheetTitle></SheetHeader><SessionList repositoryId={repo.id} selectedId={activeId} drawer onSelect={(sessionId) => { setSelectedId(sessionId); setMobileSessionsOpen(false); }} /></SheetContent></Sheet><section className="flex min-h-[65svh] min-w-0 flex-1 flex-col lg:min-h-0"><ChatMessages repository={repo} messages={messages.data ?? []} streamText={stream.streamText} loading={messages.isLoading || (!activeId && create.isPending)} onStarterPrompt={stream.send} starterDisabled={!activeId || stream.streaming} /><ChatComposer disabled={!activeId} streaming={stream.streaming} error={stream.error || messages.error?.message || sessions.error?.message || create.error?.message} onSend={stream.send} onStop={stream.stop} /></section></div>}</div>;
}
