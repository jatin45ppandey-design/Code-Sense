...
export default function AuthCallbackPage() {
  ...
  if (!user.isError && (fromOAuth || user.data)) return <main className="codesense-landing relative min-h-svh overflow-hidden"><div aria-hidden="true" className="technical-grid codesense-landing-grid pointer-events-none absolute inset-0" /><AuthTransition stage={stage} /></main>;

  if (!user.isError) return <main className="flex min-h-svh items-center justify-center p-6"><div className="w-full max-w-sm text-center"><LoaderCircle className="mx-auto size-7 animate-spin text-primary" /><h1 className="mt-4 text-lg font-semibold">Finishing sign-in</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">Confirming your GitHub account and preparing your workspace.</p></div></main>;

  return <main className="flex min-h-svh items-center justify-center p-6"><div className="w-full max-w-sm text-center">{user.isError && <AlertCircle className="mx-auto size-8 text-destructive" />}<h1 className="mt-4 text-lg font-semibold">Sign-in could not be completed</h1><p className="mt-2 text-sm leading-6 text-muted-foreground">CodeSense could not confirm your authenticated session. Return to the login page and try GitHub sign-in again.</p><Button className="mt-5" onClick={retryLogin}><RotateCcw />Try sign-in again</Button></div></main>;
}
