...
import jatin.backend.DTO.ChatSessionResponse;
import jatin.backend.DTO.CreateChatSessionRequest;
...
public class ChatService {
    ...
    public ChatSessionResponse createSession(UUID userId, CreateChatSessionRequest request) {
        ...
        if (repo.getIndexStatus() != IndexStatus.READY) {
            throw new BadReqException("Repository must be indexed before chatting");
        }
    ...
    public SseEmitter streamReply(UUID userId, UUID sessionId, String userContent) {
        ...
        if (repo.getIndexStatus() != IndexStatus.READY) {
            throw new BadReqException("Repository is not ready for chat");
        }
