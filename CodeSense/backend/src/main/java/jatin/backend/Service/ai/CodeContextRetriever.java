    ...
    private final CitationMapper citationMapper; 
     ...
     public RetrievedContext retrieve(UUID repositoryId, String question) {
        var filter = new FilterExpressionBuilder()
                .eq(RagSettings.METADATA_REPOSITORY_ID, repositoryId.toString())
                .build();
        ...
        java.util.List<Document> searchResults;
        try {
            searchResults = vectorStore.similaritySearch(search);
        } catch (RuntimeException exception) {
            throw AiProviderErrors.sanitize(exception);
        }

        var documents = searchResults.stream()
                .filter(document -> repositoryId.toString().equals(
                        String.valueOf(document.getMetadata().get(RagSettings.METADATA_REPOSITORY_ID))))
                .toList();
